...
description: 'Creates an Architecture Decision Record (ADR) documenting a significant technical decision, its context, alternatives considered, and consequences. Every major technical choice should have an ADR. Arguments: [title] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/architecture-decision/SKILL.md -->
...
5. If yes:
   ...
   - Append each missing section to the ADR file using the apply_patch tool.
...
4.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via spawn_agent to validate the drafted ADR:
   ...
   - Spawn `agent_type: [primary specialist]` with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
...
4.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via spawn_agent using gate **TD-ADR** (`.claude/docs/director-gates.md`):
