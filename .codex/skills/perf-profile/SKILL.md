---
name: perf-profile
description: 'Structured performance profiling workflow. Identifies bottlenecks, measures against budgets, and generates optimization recommendations with priority rankings. Arguments: [system-name or ''full'']'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/perf-profile/SKILL.md -->

## Delegation (Codex)

Run this skill as the `performance-analyst` role: open with a `spawn_agent` call setting
`agent_type: performance-analyst`, and pass the user's request through as the `message`.
