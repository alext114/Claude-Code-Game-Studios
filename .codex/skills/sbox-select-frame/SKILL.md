...
description: 'Select one or more GameObjects in the s&box editor via MCP and frame the camera on them. Use to focus editor attention on a specific object before configuring it, or to verify scene placement visually. Arguments: <ObjectName or ''tag:tagname''> (e.g., ''Player'', ''Enemy_01'', ''tag:nav-agent'')'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/sbox-select-frame/SKILL.md -->
