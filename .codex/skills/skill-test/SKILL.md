---
name: skill-test
description: 'Validate skill files for structural compliance and behavioral correctness. Three modes: static (linter), spec (behavioral), audit (coverage report). Arguments: static [skill-name | all] | spec [skill-name] | category [skill-name | all] | audit'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/skill-test/SKILL.md -->
...
**FAIL** if `allowed-tools` includes `apply_patch` or `apply_patch` but no ask-before-write language is found.
