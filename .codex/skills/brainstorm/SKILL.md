---
name: brainstorm
description: 'Guided game concept ideation — from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration. Arguments: [genre or theme hint, or ''open''] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/brainstorm/SKILL.md -->
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via spawn_agent in parallel before moving to Phase 5. Issue both Task calls simultaneously — do not wait for one before starting the other.**
...
**After identifying biggest technical risks, spawn `technical-director` via spawn_agent using gate TD-FEASIBILITY (`.claude/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via spawn_agent using gate PR-SCOPE (`.claude/docs/director-gates.md`).**
