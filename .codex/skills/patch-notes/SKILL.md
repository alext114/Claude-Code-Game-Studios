---
name: patch-notes
description: 'Generate player-facing patch notes from git history, sprint data, and internal changelogs. Translates developer language into clear, engaging player communication. Arguments: [version] [--style brief|detailed|full]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/patch-notes/SKILL.md -->

## Delegation (Codex)

Run this skill as the `community-manager` role: open with a `spawn_agent` call setting
`agent_type: community-manager`, and pass the user's request through as the `message`.
