---
name: content-audit
description: 'Audit GDD-specified content counts against implemented content. Identifies what''s planned vs built. Arguments: [system-name | --summary | (no arg = full audit)]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/content-audit/SKILL.md -->

## Delegation (Codex)

Run this skill as the `producer` role: open with a `spawn_agent` call setting
`agent_type: producer`, and pass the user's request through as the `message`.
