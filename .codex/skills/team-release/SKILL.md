---
name: team-release
description: 'Orchestrate the release team: coordinates release-manager, qa-lead, devops-engineer, and producer to execute a release from candidate to deployment. Arguments: [version number or ''next'']'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/team-release/SKILL.md -->
...
- `agent_type: release-manager` — Release branch, versioning, changelog, deployment
- `agent_type: qa-lead` — Test sign-off, regression suite, release quality gate
- `agent_type: devops-engineer` — Build pipeline, artifacts, deployment automation
- `agent_type: security-engineer` — Security audit for online/multiplayer/data features
- `agent_type: analytics-engineer` — Telemetry event verification and dashboard readiness
- `agent_type: community-manager` — Patch notes and launch communication
- `agent_type: producer` — Go/no-go decision, stakeholder communication
- `agent_type: network-programmer` — Netcode stability sign-off (invoke if game has multiplayer)
