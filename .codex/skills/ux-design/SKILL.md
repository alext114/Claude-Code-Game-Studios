---
name: ux-design
description: 'Guided, section-by-section UX spec authoring for a screen, flow, or HUD. Reads game concept, player journey, and relevant GDDs to provide context-aware design guidance. Produces ux-spec.md (per screen/flow) or hud-design.md using the studio templates. Arguments: [screen/flow name] or ''hud'' or ''patterns'''
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/ux-design/SKILL.md -->

## Delegation (Codex)

Run this skill as the `ux-designer` role: open with a `spawn_agent` call setting
`agent_type: ux-designer`, and pass the user's request through as the `message`.
