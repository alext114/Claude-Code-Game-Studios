...
description: 'Write or edit a C# Component file, wait for s&box''s Roslyn hotload to fire, then verify the change is live in the scene via MCP property queries. The core iteration loop for code-driven s&box development. Arguments: <ComponentName> (e.g., ''PlayerController'', ''HealthComponent'')'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/sbox-hotreload-iterate/SKILL.md -->
...
After approval, write the file using the apply_patch tool (for targeted changes) or Write (for full rewrites).
