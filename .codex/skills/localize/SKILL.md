---
name: localize
description: 'Full localization pipeline: scan for hardcoded strings, extract and manage string tables, validate translations, generate translator briefings, run cultural/sensitivity review, manage VO localization, test RTL/platform requirements, enforce string freeze, and report coverage. Arguments: [scan|extract|validate|status|brief|cultural-review|vo-pipeline|rtl-check|freeze|qa]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/localize/SKILL.md -->

## Delegation (Codex)

Run this skill as the `localization-lead` role: open with a `spawn_agent` call setting
`agent_type: localization-lead`, and pass the user's request through as the `message`.
