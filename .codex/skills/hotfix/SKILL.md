---
name: hotfix
description: 'Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix branch, tracks approvals, and ensures the fix is backported correctly. Arguments: [bug-id or description]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/hotfix/SKILL.md -->
...
- `agent_type: lead-programmer` — Review the fix for correctness and side effects
- `agent_type: qa-tester` — Run targeted regression tests on the affected system
- `agent_type: producer` — Approve deployment timing and communication plan
