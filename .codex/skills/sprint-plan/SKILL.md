---
name: sprint-plan
description: 'Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs. Arguments: [new|update|status] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/sprint-plan/SKILL.md -->
...
Before finalising the sprint plan, spawn `producer` via spawn_agent using gate **PR-SPRINT** (`.claude/docs/director-gates.md`).
...
Use `shell` to look for `production/qa/qa-plan-sprint-[N].md` or any file in `production/qa/` referencing this sprint number.
