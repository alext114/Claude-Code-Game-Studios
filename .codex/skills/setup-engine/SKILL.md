---
name: setup-engine
description: 'Configure the project''s game engine and version. Pins the engine in CLAUDE.md, detects knowledge gaps, and populates engine reference docs via web_search when the version is beyond the LLM''s training data. Arguments: [engine] | [engine version] | refresh | upgrade [old-version] [new-version] | no args for guided selection'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/setup-engine/SKILL.md -->
...
All s&box agents read `docs/engine-reference/sbox/VERSION.md` and use web_search to
...
Use web_search and web_search to locate the official migration guide between
...
=====================
Engine:          s&box [version]
Knowledge Risk:  HIGH (API evolved significantly post-cutoff — always web_search)
