---
name: map-systems
description: 'Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index. Arguments: [next | system-name] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/map-systems/SKILL.md -->
...
**After dependency mapping is approved, spawn `technical-director` via spawn_agent using gate TD-SYSTEM-BOUNDARY (`.claude/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via spawn_agent using gate PR-SCOPE (`.claude/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via spawn_agent using gate CD-SYSTEMS (`.claude/docs/director-gates.md`).**
