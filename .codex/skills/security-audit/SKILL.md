---
name: security-audit
description: 'Audit the game for security vulnerabilities: save tampering, cheat vectors, network exploits, data exposure, and input validation gaps. Produces a prioritised security report with remediation guidance. Run before any public release or multiplayer launch. Arguments: [full | network | save | input | quick]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/security-audit/SKILL.md -->

## Delegation (Codex)

Run this skill as the `security-engineer` role: open with a `spawn_agent` call setting
`agent_type: security-engineer`, and pass the user's request through as the `message`.
