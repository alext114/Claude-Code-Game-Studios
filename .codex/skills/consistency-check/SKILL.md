...
description: 'Scan all GDDs against the entity registry to detect cross-document inconsistencies: same entity with different stats, same item with different values, same formula with different variables. Grep-first approach — reads registry then targets only conflicting GDD sections rather than full document reads. Arguments: [full | since-last-review | entity:<name> | item:<name>]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/consistency-check/SKILL.md -->
