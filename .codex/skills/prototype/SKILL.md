---
name: prototype
description: 'Rapid prototyping workflow. Skips normal standards to quickly validate a game concept or mechanic. Produces throwaway code and a structured prototype report. Arguments: [concept-description] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/prototype/SKILL.md -->

## Delegation (Codex)

Run this skill as the `prototyper` role: open with a `spawn_agent` call setting
`agent_type: prototyper`, and pass the user's request through as the `message`.
...
Spawn `creative-director` via spawn_agent using gate **CD-PLAYTEST** (`.claude/docs/director-gates.md`).
...
This skill produces throwaway work that must not land on the working branch. Create a scratch git worktree before you begin and do the work there.
