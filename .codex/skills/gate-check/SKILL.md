---
name: gate-check
description: 'Validate readiness to advance between development phases. Produces a PASS/CONCERNS/FAIL verdict with specific blockers and required artifacts. Use when user says ''are we ready to move to X'', ''can we advance to production'', ''check if we can start the next phase'', ''pass the gate''. Arguments: [target-phase: systems-design | technical-setup | pre-production | production | polish | release] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/gate-check/SKILL.md -->
...
### Artifact Checks
- Use `shell` and `read_file` to verify files exist and have meaningful content
...
- For localization checks: `shell` for hardcoded strings in `src/`
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via spawn_agent using the parallel gate protocol from `.claude/docs/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.
