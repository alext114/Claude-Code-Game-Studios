---
name: story-done
description: 'End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for GDD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint. Arguments: [story-file-path] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/story-done/SKILL.md -->
...
- **File existence check**: `shell` for files the story said would be created.
- **Test pass check**: if a test file path is mentioned, run it via `shell`.
- **No hardcoded values check**: `shell` for numeric literals in gameplay code
  paths that should be in config files.
- **No hardcoded strings check**: `shell` for player-facing strings in `src/`
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `tests/unit/` for a test file or function name that
     matches the criterion's subject (use `shell` and `shell`)
...
exact required file path. Use `shell` to check that exact path. If the exact path is not
...
required path. Use `shell` to check that exact path first, then search
...
1. **GDD rules check**: Using the current requirement text from `tr-registry.yaml`
   ...
   `shell` the implemented files for key function names, data structures, or class
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   ...
   exists). `shell` for patterns explicitly forbidden in the ADR.

4. **Hardcoded values check**: `shell` the implemented files for numeric literals
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via spawn_agent using gate **QL-TEST-COVERAGE** (`.claude/docs/director-gates.md`).
...
Spawn `lead-programmer` via spawn_agent using gate **LP-CODE-REVIEW** (`.claude/docs/director-gates.md`).
