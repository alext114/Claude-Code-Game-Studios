---
name: create-epics
description: 'Translate approved GDDs + architecture into epics — one epic per architectural module. Defines scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories — run $create-stories [epic-slug] after each epic is created. Arguments: [system-name | layer: foundation|core|feature|presentation | all] [--review full|lean|solo]'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/create-epics/SKILL.md -->

## Delegation (Codex)

Run this skill as the `technical-director` role: open with a `spawn_agent` call setting
`agent_type: technical-director`, and pass the user's request through as the `message`.
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via spawn_agent using gate **PR-EPIC** (`.claude/docs/director-gates.md`).
