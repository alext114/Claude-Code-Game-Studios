...
description: 'Browse and search available Component types in the active s&box TypeLibrary via MCP. Use to find what built-in components are available before attaching them, or to verify a custom C# Component has compiled successfully. Arguments: [optional: search keyword, e.g., ''physics'', ''nav'', ''render'', ''collider'']'
---

<!-- GENERATED by tools/codex-sync — do not edit. Source: .claude/skills/sbox-discover-components/SKILL.md -->
